<template>
  <div :class="cn('relative w-full', className)">
    <div class="relative">
      <Input v-model="inputValue" type="text" :placeholder="placeholder" inputmode="numeric"
        :class="cn('w-full pr-10', !formattedValue && 'text-muted-foreground')" :disabled="disabled"
        @beforeinput="handleBeforeInput" @input="handleInput" @blur="handleInputBlur" @keydown.enter.prevent="handleInputBlur" />
...
function handleBeforeInput(event: InputEvent) {
  ...
  // Preserve the trailing date digits when users edit a full date in the middle.
  if (currentDigits.length === 8 && selectionStart === selectionEnd) {
