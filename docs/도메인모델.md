...
### EmployeePosition
_enum_
- 값: `ASSOCIATE(사원)`, `STAFF(선임)`, `LEADER(책임)`, `MANAGER(팀장)`, `SENIOR_MANAGER(수석)`, `DIRECTOR(이사)`, `TECHNICAL_DIRECTOR(기술이사)`, `MANAGING_DIRECTOR(상무)`, `VICE_PRESIDENT(부사장)`, `PRESIDENT(사장)`
