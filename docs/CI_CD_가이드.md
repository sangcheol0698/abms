...
- `.github/workflows/deploy-api-dev.yml`
  ...
  - `./gradlew :abms-api-boot:bootJar`
  - 개발 서버에 `abms-api.jar` 업로드
  - `ops/observability` 업로드
  - API 재기동 후 `/actuator/health` 확인
  - Prometheus target `abms-api` 가 `UP` 인지 확인
  - 성공 시 `deploy-api-dev-YYYYMMDD-HHMMSS-<shortsha>` 형식의 git 태그 생성
- `.github/workflows/deploy-batch-dev.yml`
  - `workflow_dispatch` 로 수동 실행
  - 입력값 `ref` 로 배포 대상 브랜치 또는 커밋 지정
  - 입력값 `create_tag` 로 배포 태그 생성 여부 지정
  - `./gradlew :abms-batch-boot:bootJar`
  - 개발 서버에 `abms-batch.jar` 업로드
  - API 재기동이나 observability 재기동 없이 배포만 수행
  - 성공 시 `deploy-batch-dev-YYYYMMDD-HHMMSS-<shortsha>` 형식의 git 태그 생성
...
- 개발 서버의 `DEV_SERVER_APP_DIR` 에 최신 `abms-api.jar`, `abms-batch.jar` 를 각각 배포할 수 있어야 합니다.

## 권장 운영 방식
- PR 검증은 `CI` 로만 처리합니다.
- 프런트 또는 API 코드 변경은 Actions 탭에서 `Deploy API Dev` 를 실행합니다.
